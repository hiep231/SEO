		...
		<div className="group/container relative md:sticky md:top-20 md:h-fit">
					...
					{product.imgUrls.map((src, i) => (
						<CarouselItem key={`carousel-item-${i}`}>
							<Image
								...
								src={src}
								alt={`${product.name[locale]} - Ảnh ${i + 1} - Mua chính hãng giá tốt`}
								title={`${product.name[locale]} - Xem chi tiết`}
								width={512}
								height={512}
								priority={i === 0}
							/>
			...
			<div className="grid grid-cols-4 gap-2 md:gap-4">
				{product.imgUrls.map((imgUrl, i) => (
					<Image
						key={`${product.name[locale]}-thumb-${i + 1}`}
						role="button"
						className={cn(
						...
						src={imgUrl}
						alt={`${product.name[locale]} - Ảnh ${i + 1}`}
